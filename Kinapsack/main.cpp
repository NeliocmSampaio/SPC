 ...
 * Codigo do Problema: KNAPSACK  Problema: The Knapsack Problem
 *
 *  O algoritmo utiliza a solução com programação dinâmica para o problema da
 *  mochila. Calcula-se o máximo que se pode carregar em uma mochila de capacidade i,
 *  para i de 0 até s (s a capacidade da mochila original), considerando todos os itens
 *  (máximo entre [o valor do item j (caso ele caiba na mochila i) + o valor máximo carregado
 *  em uma mochila de capacidade j-peso do item atual] e [o valor carregado pela mochila de
 *  capacidade i até o item j-1]). Todos os calculos são guardados em uma matriz para que
 *  não seja necessário recalculo.
 * */
